        ...
        if(produtosCarrinho.containsKey(codigo)){
            ...
            if (produtosDoCodigo.get(0).getNome().equals(produto.getNome())){
                if (produto instanceof ProdutoUnitario) {
                    ...
                    pdt.setQuatidadeQuilos(pdt.getQuatidadeQuilos() +  quantidade);
            ...
            if (produto instanceof ProdutoUnitario) {
                ...
                ((ProdutoQuilo) produto).setQuatidadeQuilos(quantidade);
                produtosDoCodigo.add(produto);
            }
        ...
        if(this.produtosCarrinho.size() > 0){
            ...
            while(it.hasNext()){
                ...
                if(produto_devolucao instanceof ProdutoUnitario){
                    Estoque.adicionarProduto(produto_devolucao, quantidade);
                }
                if(produto_devolucao instanceof ProdutoQuilo){
                    ProdutoQuilo prodKg = Estoque.ObtenhaProdutoQuiloTemporario(produto_devolucao);
                    double kilos = prodKg.getQuatidadeQuilos();
                    Estoque.adicionarProduto(prodKg, kilos);
                }
            }
            this.produtosCarrinho.clear();
            System.out.print("##   COMPRA CANCELADA, VOCÊ NÃO POSSUI MAIS PRODUTOS NO SEU CARRINHO!   ##");
        ...
        if(produtosCarrinho.size() > 0){
            ...
            while (it.hasNext()) {
                ...
                if (p instanceof ProdutoQuilo) {
                    ...
                    System.out.println("Quilos: " + pdt.getQuatidadeQuilos() + "kg\n");
                }
        ...
        while(itMap.hasNext()){
            ...
            while(produtos.hasNext()){
                ...
                if(produtoList instanceof ProdutoQuilo){
                    ...
                    valorPeso = produtokg.getPreco();
                    ktdKilo = produtokg.getQuatidadeQuilos();
                ...
                //Fazer a variavel "valorTotal" receber o valor do calculo por Unidade
                    ...
                    valorTotal += Balanca.calcularValorPorItem(produtounit.getPreco(), quantidade);
                }
            }
        }
        return valorTotal;
